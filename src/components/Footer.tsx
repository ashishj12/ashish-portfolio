...
export default function Footer() {
  return (
    <footer className="relative border-t border-[var(--line)] bg-[var(--bg-soft)]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 md:px-8 py-12 sm:py-14">
        ...
        <div className="mt-10 sm:mt-12 pt-6 border-t border-[var(--line)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs mono-eyebrow text-[var(--ink-faint)]">
          <span className="break-words">
            © {new Date().getFullYear()} {profile.name}
          </span>
          <span className="break-words">Built by {profile.name}</span>
        </div>
      </div>
    </footer>
  );
